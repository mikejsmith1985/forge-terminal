...
export const PORTFOLIO_APPS = [
  ...
  },
  {
    "slug": "recall-radar",
    "name": "Recall Radar",
    "tagline": "Ask whether a symptom in your car is a known defect, and get an answer whose every quote has been checked against the NHTSA record it came from.",
    "headline": "Adding embeddings made the evaluation worse. The numbers are on the page, and so is why.",
    "summary": "A retrieval-augmented question answering system over public NHTSA data: owner complaints, recall campaigns, and the defect investigations that connect them, for a specific vehicle. Hybrid retrieval — pgvector for meaning, Postgres full-text for keywords, fused by reciprocal rank — feeds Claude a schema that makes citations data rather than prose. Every quote is then checked as a literal substring of its record. A quote that is not there is dropped and counted; an answer with no surviving quote is returned as not grounded and its prose is discarded with it, because text that reads as an answer will be read as one whatever flag sits beside it.",
    "accent": "#1a56db",
