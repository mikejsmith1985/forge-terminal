/**
 * useVault.js ΓÇö React hook for the Forge Vault secret management feature.
 *
 ...
 * Secret values are NEVER stored in component state ΓÇö the API never returns them.
 */
...
export function useVault() {
   ...
   * Entry objects contain metadata only ΓÇö no secret values are returned.
   */
  ...
  const toggleAutoInject = useCallback(async (entryId, shouldAutoInject) => {
    // Optimistic update ΓÇö revert on failure
   ...
   * Values are never stored in hook state ΓÇö each call fetches fresh from the
