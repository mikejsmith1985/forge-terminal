/**
 * VaultPanel.jsx ΓÇö Compact modal panel for the Forge Vault secret manager.
 *
 ...
 *
 * Secret values are NEVER displayed ΓÇö only metadata (names, env var names, flags).
 *
 ...
 * keeps the inject logic simple ΓÇö each env var is still injected individually.
 */
 ...
 * "OpenAI API Key" ΓåÆ "OPENAI_API_KEY"
 *
 ...
 *
 * Reveal state is local to the card ΓÇö the decrypted value never enters
  ...
  return (
    <div className="vp-entry-card" role="listitem">
      ...
      <div className="vp-entry-reveal-actions">
        ...
        >
          {isRevealing ? (
            <span className="vp-spinner">Γƒ│</span>
          ) : isRevealed ? (
            <><EyeOff size={12} /> Hide</>
          ) : (
            <><Eye size={12} /> Reveal</>
      ...
      {/* Revealed value ΓÇö only rendered while active */}
      {isRevealed && (
 ...
 * Intentionally has NO button ΓÇö the toolbar "Add Secret" button is the single call-to-action.
 */
  ...
  return (
    <div className="vp-empty-state">
      ...
      <p className="vp-empty-state-subtext">
        Forge auto-injects them into every new terminal session ΓÇö your agents never see the raw values.
      </p>
    </div>
  )
}
 ...
 *     (e.g. "GitHub" ΓåÆ GITHUB_USERNAME + GITHUB_PASSWORD), submitted as two
...
function AddSecretForm({ isAdding, onSubmit, onCancel }) {
  ...
  // Visibility toggles ΓÇö separate states so username and password reveal independently
  ...
  const handleFormSubmit = useCallback(async (submitEvent) => {
    ...
    if (secretType === SECRET_TYPE_API_TOKEN) {
      ...
      // Credential path ΓÇö submit username and password as two independent entries
      ...
      const usernameWasStored = await onSubmit({
        secretName: `${credentialName.trim()} ΓÇö Username`,
      ...
      const passwordWasStored = await onSubmit({
        secretName: `${credentialName.trim()} ΓÇö Password`,
  ...
  return (
    <div className="vp-add-modal-overlay" role="dialog" aria-modal="true" aria-label="Add secret">
      <form className="vp-add-modal" onSubmit={handleFormSubmit} autoComplete="off">
        ...
        {/* ΓöÇΓöÇ API Token fields ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        {secretType === SECRET_TYPE_API_TOKEN && (
          <>
        ...
        {/* ΓöÇΓöÇ Credential fields (username + password) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        ...
        {/* Description ΓÇö shared by both types */}
        <div className="vp-form-field">
        ...
        <div className="vp-form-actions">
          ...
          >
            {isAdding ? (
              ...
              <>
                <Lock size={14} />
                {secretType === SECRET_TYPE_API_TOKEN ? 'Store Securely ≡ƒöÉ' : 'Store Credential ≡ƒöÉ'}
              </>
            )}
          </button>
...
/**
 * VaultPanel ΓÇö Forge Vault secret manager rendered as a compact centered modal.
 *
...
export function VaultPanel({ isOpen, onClose, onToast }) {
  ...
  // Render nothing when closed ΓÇö avoids hidden DOM and accidental event capture
  if (!isOpen) return null

  return (
    // Backdrop ΓÇö clicking it closes the panel
    <div
      className="vault-panel-backdrop"
      onClick={onClose}
      aria-hidden="true"
    >
      {/* Dialog ΓÇö stop clicks from bubbling to the backdrop */}
      <div
      ...
      >
        ...
        <header className="vp-header">
          <div className="vp-header-left">
            ...
            <div className="vp-header-title-group">
              <h1 className="vp-header-title">≡ƒöÉ Forge Vault</h1>
        ...
        {/* ΓöÇΓöÇ Error Banner ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        {error && (
        ...
        {/* ΓöÇΓöÇ Toolbar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        ...
        {/* ΓöÇΓöÇ Entry List ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        ...
        {/* ΓöÇΓöÇ Add Secret Modal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        {isAddFormVisible && (
          <AddSecretForm
            isAdding={isAdding}
        ...
        {/* ΓöÇΓöÇ Delete Confirmation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        {entryBeingDeleted && (
          <DeleteConfirmDialog
