...
const ForgeTerminal = forwardRef(function ForgeTerminal({
  ...
  useEffect(() => {
    ...
    term.attachCustomKeyEventHandler((arg) => {
      ...
      // v3.11.7 FIX: Ctrl+V should ONLY trigger native paste event
      // Let browser's native paste event do the work (handlePaste above)
      // DO NOT try to handle clipboard API here - it creates race conditions
      if (arg.ctrlKey && arg.code === 'KeyV' && arg.type === 'keydown') {
        console.log('[Terminal] Ctrl+V pressed - letting native paste event handle it');
        // Just return true to allow the event through
        // The paste event listener (handlePaste) will fire and handle it
        return true;
      }
