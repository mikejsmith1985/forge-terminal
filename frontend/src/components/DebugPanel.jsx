...
const DebugPanel = ({ terminalRef, tabId }) => {
  ...
  const captureDiagnostics = useCallback(() => {
    ...
    const snapshot = {
      ...
      wsState: terminalRef?.getSocket?.()?.readyState ?? terminalRef?.wsRef?.current?.readyState ?? 'no-ref',
      wsStateText: getWebSocketState(terminalRef?.getSocket?.()?.readyState ?? terminalRef?.wsRef?.current?.readyState),
      terminalRows: terminalRef?.getTerminal?.()?.rows ?? terminalRef?.terminal?.rows ?? 'unknown',
      terminalCols: terminalRef?.getTerminal?.()?.cols ?? terminalRef?.terminal?.cols ?? 'unknown',
