...
test.describe('Smart Routing System', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const tourVisible = await page.getByTestId('tour-modal').isVisible().catch(() => false);
    if (tourVisible) {
      await page.getByTestId('tour-skip-button').click();
    }
  ...
  test('classifies task patterns correctly', async ({ page }) => {
    ...
    for (const testCase of patterns) {
      ...
      const response = await page.request.post('http://localhost:3000/api/routing/classify', {
        data: { 
  ...
  test('logs effectiveness data after task completion', async ({ page }) => {
    ...
    const response = await page.request.get('http://localhost:3000/api/routing/effectiveness-log');
    expect(response.ok()).toBeTruthy();
