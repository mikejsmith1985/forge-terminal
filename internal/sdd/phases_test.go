...
func TestNextPhaseAfter(t *testing.T) {
	...
	if !ok || next.Name != PhaseValidate {
		t.Fatalf("after plan = %+v ok=%v, want validate", next, ok)
	}
...
func TestPhaseTableIsOrderedAndComplete(t *testing.T) {
	wantOrder := []PhaseName{PhaseSpecify, PhaseClarify, PhasePlan, PhaseValidate, PhaseImplement}
