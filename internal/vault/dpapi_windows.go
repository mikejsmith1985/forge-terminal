// dpapi_windows.go ΓÇö Windows DPAPI key protection for the Forge Vault.
//
...
func wrapKeyWithOSCredentials(keyBytes []byte) ([]byte, error) {
	...
	returnCode, _, lastError := procCryptProtectData.Call(
		...
		0, // szDataDescr ΓÇö no description label needed
		0, // pOptionalEntropy ΓÇö no additional entropy
		0, // pvReserved ΓÇö must be NULL per MSDN
		0, // pPromptStruct ΓÇö no UI prompts
		0, // dwFlags ΓÇö current-user scope (default)
...
func unwrapKeyFromOSCredentials(wrappedKey []byte) ([]byte, error) {
	...
	returnCode, _, lastError := procCryptUnprotect.Call(
		uintptr(unsafe.Pointer(inputBlob)),
		0, // ppszDataDescr ΓÇö not needed
		0, // pOptionalEntropy ΓÇö must match what was used during protect
		0, // pvReserved ΓÇö must be NULL
		0, // pPromptStruct ΓÇö no UI prompts
		0, // dwFlags
		uintptr(unsafe.Pointer(&outputBlob)),
	)
