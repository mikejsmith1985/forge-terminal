...
var preCommitSHTemplateBody = "#!/usr/bin/env bash\n" +
	"# Forge Workflow Pre-Commit Hook (Enhanced)\n" +
	"# Generated by Forge Terminal Workflow Architect\n" +
	"# Enforces: main branch block, test file gate, CHANGELOG, naming, branch naming.\n" +
	"# Bypass with: git commit --no-verify (bypasses are logged)\n" +
...
// preCommitSHTemplate is the scaffold's body with the runtime workflow gate
// placed ahead of its own checks. Composed rather than pasted so the gate the
// scaffold writes and the gate EnsureHookInstalled merges into an existing
// scaffold hook can never drift apart — that drift is how the gate went
// missing from every scaffolded repository in the first place.
var preCommitSHTemplate = mergeLedgerGate(preCommitSHTemplateBody)
