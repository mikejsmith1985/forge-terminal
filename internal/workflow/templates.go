...
// RenderLocalReleasePS1 generates a generic local release pipeline for scaffolded projects.
// The script keeps release creation local through gh CLI so projects do not depend on Actions.
func RenderLocalReleasePS1(config WorkflowConfig) (string, error) {
	return renderTemplate("local-release-ps1", localReleasePS1Template, config)
}
...
// ──────────────────────────────────────────────────────────────────────────────
// Template: scripts/local-release.ps1
// ──────────────────────────────────────────────────────────────────────────────

var localReleasePS1Template = `# scripts/local-release.ps1
# Local release pipeline generated by Forge Terminal for {{.ProjectName}}.
# Creates GitHub releases directly with gh CLI; it never relies on GitHub Actions.
#
# Usage:
#   .\scripts\local-release.ps1 patch
#   .\scripts\local-release.ps1 minor
#   .\scripts\local-release.ps1 major
#   .\scripts\local-release.ps1 1.2.3
...
if (-not (Get-Command git -ErrorAction SilentlyContinue)) { Stop-Release "git not found in PATH" }
if (-not (Get-Command gh -ErrorAction SilentlyContinue)) { Stop-Release "gh CLI not found in PATH" }
gh auth status 2>$null
if ($LASTEXITCODE -ne 0) { Stop-Release "gh CLI is not authenticated. Run: gh auth login" }

$originalBranch = git branch --show-current
if (-not $originalBranch) { Stop-Release "Unable to determine current git branch" }
if ($originalBranch -eq "main" -or $originalBranch -eq "master") {
    Stop-Release "Create a feature branch before releasing; releases merge that branch into main."
}
...
if ($Version -match '^\d+\.\d+\.\d+$') {
...
} elseif ($Version -in @("patch", "minor", "major")) {
    $versionParts = $currentTag.TrimStart("v").Split(".")
    ...
    Stop-Release "Unknown version '$Version'. Use patch, minor, major, or X.Y.Z."
}

$releaseTag = "v$releaseVersion"
Write-ReleaseStep "Preparing $releaseTag from branch $originalBranch"

if (Test-Path "package.json") {
    Write-ReleaseStep "Updating package.json to $releaseTag"
    npm version $releaseTag --no-git-tag-version --allow-same-version
    if ($LASTEXITCODE -ne 0) { Stop-Release "npm version failed" }
}

if ($DryRun) {
    Write-Host "Dry run complete. Would release $releaseTag." -ForegroundColor Yellow
    exit 0
}

git add -A
git commit -m "chore: release $releaseTag" --allow-empty
if ($LASTEXITCODE -ne 0) { Stop-Release "release commit failed" }
git push origin $originalBranch
if ($LASTEXITCODE -ne 0) { Stop-Release "feature branch push failed" }

git checkout main
if ($LASTEXITCODE -ne 0) { Stop-Release "git checkout main failed" }
git pull origin main
if ($LASTEXITCODE -ne 0) { Stop-Release "git pull origin main failed" }
git merge $originalBranch --no-edit
if ($LASTEXITCODE -ne 0) { Stop-Release "git merge $originalBranch failed; resolve conflicts and re-run" }
git push origin main
if ($LASTEXITCODE -ne 0) { Stop-Release "main push failed" }

$existingLocalTag = git tag -l $releaseTag 2>$null
if ($existingLocalTag) { git tag -d $releaseTag | Out-Null }

$existingRemoteTag = git ls-remote --tags origin "refs/tags/$releaseTag" 2>$null
if ($existingRemoteTag) { git push origin ":refs/tags/$releaseTag" | Out-Null }

$existingRelease = gh release view $releaseTag --json tagName --jq ".tagName" 2>$null
if ($LASTEXITCODE -eq 0 -and $existingRelease) {
    gh release delete $releaseTag --yes
    if ($LASTEXITCODE -ne 0) { Stop-Release "stale GitHub release delete failed" }
}

git tag $releaseTag
if ($LASTEXITCODE -ne 0) { Stop-Release "tag creation failed" }
git push origin $releaseTag
if ($LASTEXITCODE -ne 0) { Stop-Release "tag push failed" }

if ($ReleaseNotes) {
    $notesFile = [System.IO.Path]::GetTempFileName()
    [System.IO.File]::WriteAllText($notesFile, $ReleaseNotes, [System.Text.UTF8Encoding]::new($false))
    gh release create $releaseTag --title "Release $releaseTag" --notes-file $notesFile --latest
    Remove-Item $notesFile -ErrorAction SilentlyContinue
} else {
    gh release create $releaseTag --title "Release $releaseTag" --notes "Release $releaseTag" --latest
}
if ($LASTEXITCODE -ne 0) { Stop-Release "gh release create failed" }

git checkout $originalBranch
Write-ReleaseOk "Release $releaseTag published on GitHub"
`
