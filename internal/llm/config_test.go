...
func TestDefaultConfig(t *testing.T) {
	...
	// All tiers now use Copilot CLI with model selection
	if config.Tier3.Name != "Copilot" {
		t.Errorf("Expected Tier3 name 'Copilot', got '%s'", config.Tier3.Name)
	}
...
func TestConfigLoader_GetTierConfig(t *testing.T) {
	...
	if tier3.Name != "Copilot" {
		t.Errorf("Expected Tier3 (Opus) name 'Copilot', got '%s'", tier3.Name)
	}
}
