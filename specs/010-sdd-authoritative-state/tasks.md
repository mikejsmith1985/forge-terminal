...
- [x] T015 [US1] Add a mandatory final step to each speckit phase-command skill (the global skill definitions / their repo source) that POSTs `phase-event{event:"complete", decisions:[...]}` scoped by `$env:FORGE_SESSION_ID` (FR-001b, FR-007a) — including the artifact-less Validate/Implement phases. The emit step MUST be additive: assert the skill's existing behaviour (artifact production, completion report) is unaffected (FR-010). **DONE: appended an additive "Authoritative Phase Completion Signal" section to all 6 phase SKILL.md sources at `internal/workflow/speckit/claude-skills/speckit-*` (specify/clarify/plan/tasks/analyze→validate/implement). These embed via `//go:embed all:speckit` and install to `~/.claude/skills/` (claude-skills/→.claude/skills/). Build + workflow tests green. ACTIVATION: the currently-installed global skills are the OLD copies until the new binary is built and the skills reinstalled (deployment step).**
