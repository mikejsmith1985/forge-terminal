...
- [X] T015 [US1] Scope the gate to code changes only in `internal/workflow/ticket.go`, exempting `specs/`, documentation-only, generated and vendored paths per `contracts/brief-gate.md`, to turn T010 green
- [X] T016 [US1] Update the pre-commit hook message body in `internal/workflow/hooks.go` to name `brief-published` among the required gates and point at the publishing tool
