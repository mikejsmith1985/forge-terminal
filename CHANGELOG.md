...
### Fixed
- **Pasting an image no longer spuriously renames the active terminal tab** — When an image was pasted, `see file at ${filePath}` was sent to the PTY via WebSocket. The PTY echoed that text back through `onmessage`, updating the rolling output buffer. The 1 500 ms idle debounce then ran `extractDirectory` on that buffer; if the snapshot happened to include a shell-prompt line followed by the echoed paste text, the regex could extract the current directory and trigger `updateTabTitle` — overwriting a user-defined tab name. Added `isPasteDirectorySuppressionActiveRef`: set to `true` immediately before the file path is sent (both the `clipboard.read()` path and the `clipboardData` fallback), and cleared after 3 000 ms, gating the `extractDirectory` call so it is skipped during the paste window.
