...
### Added
- **Forge-branded favicon and Windows EXE icon** — replaced the generic placeholder favicon with a purpose-built indigo-gradient mark (bold "F" + flame accent). Source SVG lives at `frontend/public/favicon.svg`; PNG/ICO variants are generated by `scripts/generate-favicons.mjs` (uses `sharp`, already a transitive dependency). The same icon is now embedded into `fterm.exe` via `cmd/forge/forge.ico` so Windows shortcuts and the taskbar show the Forge logo.
