...
### Added
- **The portfolio now carries Recall Radar, a grounded RAG system over public NHTSA records, as the second entry in the depth tier** — Every other entry on the page argues that an agent's output can be held to a standard; this one carries that argument into retrieval-augmented generation, where "the model said so" is most often allowed to stand as evidence. The entry leads with the number that makes it credible rather than the one that flatters it: adding embeddings made the retrieval evaluation worse, meaning search scored zero against every record, and the README explains why before it explains the fix. Four source-derived replica screens follow the shipped React client token for token: the grounded answer with its dropped-citation count and the retrieved-but-unquoted recall labelled unverified, the opened record with the quoted span marked at the character offsets the verifier matched, the search results each carrying their meaning rank, keyword rank and fused score, and the evaluation table with both pools and the zeros left in. The replicas are held to the product's own rules: citation offsets are found in the record body at build time rather than typed, so the highlight and the quote cannot disagree; fused scores are computed from the ranks beside them and the list is sorted by the result; and the six evaluation rows are compared against the committed `eval/results.json` in the product repository whenever it is checked out. Every record number, narrative, quote and rank is invented, with record numbers chosen outside the ranges NHTSA issues; only the vehicle, the symptom and the measured evaluation are real.
