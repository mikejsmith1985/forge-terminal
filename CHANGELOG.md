...
- **Resume card (ID 7) now uses `claude --continue` so enforcement macro fires correctly** — The card previously ran `claude --resume`, which shows an interactive session picker. The enforcement macro fired after 4500 ms while the picker was still open — it arrived as keyboard input that acted as a search filter, leaving only sessions whose title matched "SYSTEM INJECTION". Switching to `claude --continue` (non-interactive, resumes last session automatically) allows the macro to fire after claude is ready. Users who want to pick a specific session from history can type `claude --resume` manually. Migration upgrades existing installs.
- **Purple "Another device controls this terminal" banner no longer shows on launch** — `SESSION_JOINED` with `isActiveDevice: false` is sent on every reconnect before the backend's deferred session cleanup runs, making it a false positive. The banner timer (`setTimeout(() => setIsActiveDevice(false), 600)`) has been removed from the `SESSION_JOINED` handler. The banner now only appears on explicit `CONTROL_TRANSFERRED`, which is sent only when a real second device calls `take_control` — an unambiguous signal. The in-terminal hint message is preserved.
- **Companion QR code now encodes the Named Cloudflare Tunnel URL when that method is selected** — The v7.10.1 fix that routed the QR base URL through Tailscale for "private network" access has been reverted for the Named method. Tailscale is only used as the QR host when the user explicitly picks the Tailscale method. For Named Cloudflare Tunnel the QR must encode the public stable tunnel URL, which phones can reach from any network.
- **Companion panel layout (Named method)** — QR code now renders above the Reconfigure button rather than below it, matching the expected flow: QR → Reconfigure → Change method + Tunnel health → Disable.
