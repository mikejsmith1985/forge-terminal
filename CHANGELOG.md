...
- **Stale-closure in `applyTunnelUrl` resolved** — the function now reads `companionHost` and `tunnelUrl` from refs updated on every render rather than from the closed-over render snapshot, so late-arriving tunnel-status fetch responses make decisions against current state.
- **"Another device controls this terminal" false-positive banner** — the SESSION_JOINED debounce timer was 600ms. Over Cloudflare tunnels, the round-trip for the old WebSocket connection to close and CONTROL_GRANTED to arrive can exceed 600ms, causing the banner to show and stick even when no other device is connected. Increased to 2000ms.
- **Tunnel security note added** — the companion card now shows a brief note when a Cloudflare tunnel is active, clarifying that the 64-character mobile token is the authentication barrier rather than the tunnel URL alone.
