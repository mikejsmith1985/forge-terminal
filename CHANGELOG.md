...
### Added
- **The portfolio now carries Recall Radar, a grounded RAG system over public NHTSA records, as the second entry in the depth tier** — Every other entry on the page argues that an agent's output can be held to a standard; this one carries that argument into retrieval-augmented generation, where "the model said so" is most often allowed to stand as evidence. The entry leads with the number that makes it credible rather than the one that flatters it: adding embeddings made the retrieval evaluation worse, meaning search scored zero against every record, and the README explains why before it explains the fix. Four source-derived replica screens follow the shipped React client token for token: the grounded answer with its dropped-citation count and the retrieved-but-unquoted recall labelled unverified, the opened record with the quoted span marked at the character offsets the verifier matched, the search results each carrying their meaning rank, keyword rank and fused score, and the evaluation table with both pools and the zeros left in. Unlike every other product on the site, the screens show real data: NHTSA records are public, and the session on them — the question, Claude's answer, its ten verified citations, the records they name, the search ranks and the evaluation — was captured from the running product and reproduced exactly, nothing invented or tidied. The replicas are held to the product's own rules by the test suite, which re-runs the verifier on the captured output: every citation's offsets must spell its quote inside its record, every fused score must follow from the ranks beside it, and the six evaluation rows are compared against the committed `eval/results.json` in the product repository whenever it is checked out. The thesis strip at the top of the page now carries `RAG` and `pgvector` chips, so the signal reaches the first screen rather than the fourth.
...
### Fixed
- **The change brief only ever appeared after a page reload, which in practice meant only when a second Forge instance took over** — the brief panel from the comprehension-first workflow was published correctly, stored correctly, and gated correctly, but the frame carrying it live to the tab was dropped on arrival. The terminal component hands dashboard control messages up to the app through a short list of allowed message types, and `CHANGE_BRIEF` was never added to that list, so the frame parsed as JSON, matched nothing, and vanished; the publishing tool still reported `rendered: true`, because that only proves a socket existed. The only path that showed a brief was the fetch on page load, which is why it surfaced exactly when a newer dev build took over the port and forced the browser to reload — the defect looked like a feature of running two instances. This is the third message type swallowed by that inline check (the specs/013 worktree offer went the same way), so the list now lives in `frontend/src/utils/dashboardControlMessages.js` with a unit test that names every type the backend pushes, and the terminal calls it instead of comparing strings. Proven by a Playwright test that dispatches a `CHANGE_BRIEF` frame on the active tab's real socket and reads the headline out of the panel — the earlier end-to-end test only ever wrote the brief to disk and exercised the reload path, which is why a green suite coexisted with a feature that did not work — and by a second test reading the xterm buffer model to confirm the frame never leaks into the terminal as output.
- **A dev build made from a fresh worktree served a blank page** — every request under `/assets/` answered 404 while the bundle was demonstrably inside the binary. The tracked folder `frontend/public/Assets` carried a capital A; Windows treats that as the same folder Vite writes its lowercase `assets/` bundle into, so on disk the folder kept whichever name was created first, and in a fresh checkout that was the capital. The Go embed records the on-disk name, and the embedded filesystem is case-sensitive, so the browser asked for `assets/` and the binary only knew `Assets/`. The main checkout happened to have the lowercase folder from an older build, which is why the same script worked there and hid the problem. The folder is now tracked in lowercase, matching the Vite output it shares a name with, so the build is the same in any checkout.
...
### Fixed
- **The commit gate had never run anywhere, because it was installed in a directory git had been told to ignore** — Forge's own scaffold sets `core.hooksPath` to `.forge/hooks` and writes its "Enhanced" pre-commit hook there. The ledger gate — the hook that refuses a commit without recorded gates and a change brief — was written into `.git/hooks`, which git stops reading the moment `core.hooksPath` is set. So in every scaffolded repository, this one included, each commit ran the scaffold's checks alone; "Pre-commit checks passed" was that hook talking, and the release notes two versions ago that described the gate refusing a stale ticket were describing the CLI, not the commit path. The proof was watching another project commit and push code with no brief and no refusal. The installer now asks git where hooks live, and when Forge's own scaffold hook is already there it puts the gate inside it — ahead of the scaffold's checks, because that hook ends in its own `exit 0` and anything appended after it would never run. The gate falls through on success so the scaffold's checks still follow. A hook written by some other tool is left alone as before, but no longer silently: the gate-record tool says so in its result, since a gate that quietly failed to install is indistinguishable from one that passed. Forge now exports the path of its own running binary into every tab as `FORGE_BIN`, and the hook prefers that over a search of PATH — a bare `forge` on PATH is no longer tried at all, because an unrelated npm package answers to that name and the gate then blocked every commit with a message about nothing. Outside a Forge tab with no `fterm` on PATH the gate warns loudly and stands down rather than refusing work Forge was never part of. There is a `forge workflow hooks` command that installs or merges the gate and reports where it went; the two installer scripts now delegate to it instead of carrying a third copy of the hook, since three hand-maintained copies is how the drift began. Existing repositories migrate automatically the next time any gate is recorded, which means their next commit is genuinely gated for the first time: expect a refusal until the branch's gates and brief are recorded. Proven by running the merged hook under a real shell with a stub binary — a failing preflight blocks before the scaffold's checks, a passing one falls through to them — and by this very commit, which is the first in this repository the gate has actually checked.
...
### Fixed
- **The change brief never appeared, and the commit gate was passing on stale tickets** — Both halves of the comprehension-first workflow were silently inert, and the reason was the same in each case: nothing tied a request to the tab it came from. Publishing a brief returned `rendered: false` every time, because the tool's session id was optional and nothing told the agent to pass one, so the broadcast had no tab to reach; the reload fallback then looked for the brief in Forge's own working directory, which launched from a shortcut is `system32`, and answered "nothing published" for every tab. Meanwhile with two projects open the ledger and the brief were being written into whichever bound repository the resolver found first — this session's own gates landed in a different project entirely — and the commit in the right project still passed, because a finished ticket from earlier work was lying in its `.forge/` and preflight checked gate names alone. A gate any old ticket satisfies is a formality, which is the exact failure this workflow was built to replace. Three changes, each proven by a failing test first. The three project-writing tools now take the calling tab's `FORGE_SESSION_ID` — the identity Forge already exports into every terminal — and resolve the project from that tab's own binding, so two open repositories can no longer receive each other's state; a brief that renders nowhere now says why in its result rather than reporting a bare false. A ticket is stamped with the branch it was opened on, read from `.git/HEAD` directly so it works in a unit test and in a hook shell without git on the path, and preflight refuses a ticket from any other branch, naming both branches; a ticket with no branch inside a repository is refused as stale. The reload path reads the tab's project through the same resolver. The workflow skill is updated to pass the session id on every call, because a fix in the server that agents are never told to use is no fix at all. One consequence to know about: a ticket recorded before this release carries no branch and will be refused on its first commit; start a fresh `taskId` and it opens a new ticket for the branch.
...
### Fixed
- **Command cards pasted the prompt but never ran it** — The AI-CLI cards (Claude/Copilot/Google, in Fresh, Resume and Enforced form) deliver their workflow prompt by pasting it into the CLI and then sending Enter. The Enter was sent after a fixed 50 ms pause, and that pause was a race the card lost roughly a third of the time: a modern AI CLI does not act on a paste the moment the closing marker arrives, it assembles the text and then redraws its input box as a `[Pasted text +N lines]` placeholder. An Enter arriving before that assembly finishes is read as one more line *inside* the paste rather than as "submit", so the prompt sat in the input box unsent and the card looked like it had done nothing. The intermittency is what made it hard to see — and what made it read as a fresh regression rather than a long-standing race that had drifted the wrong way as the CLIs got heavier. Forge now stops guessing at a delay and watches the terminal instead: after writing the paste it waits for the CLI to stop redrawing, which is the observable proof that the paste has landed, and only then sends Enter. The wait is capped at 2 s so a CLI that never goes quiet — a spinner, a progress bar — cannot hold the Enter forever; a late submit is recoverable, a missing one is the bug. The evidence for the diagnosis was two logs read side by side: `~/.forge/logs/macro.log` recorded thirteen recent deliveries as `delivered=true`, while Claude Code's own prompt history recorded a matching submission for only eight of them — and one of the missing ones turned up two and a half minutes later, when the prompt was submitted by hand. That gap is now closed at the source and made visible at the same time: every delivery logs a `settle=` field recording how long the Enter waited, so the next report of this shape can be read out of the log rather than guessed at. The same fix covers the SDD phase injector, which shares the delivery path.
...
### Changed
- **The portfolio opens with something built, not an argument** — Three reviewers independently said the page buried its own product, and measuring it settled the question: 19,416 pixels, 4,772 words, and the first image 2,548 pixels down — roughly three screens of prose before a reader saw anything that had been made. The page now opens with a role, the hook, the stack, and an artefact at 561 pixels: U2 Counter answering a trade-counter question against a 1960s MultiValue ERP, beside the line about never having heard of a Pick database on Friday afternoon. The opening statement went from 107 words to 45; the long version still exists further down, where somebody already interested will find it. The reviewers' other warning — that claiming everything is checkable reads as self-mythologising if the repositories are thin — turned out to be inverted here: every number was understated. 800+ tests are 896, 40 browser specs are 42, 565 releases are 569, 92 merged changes are 93. All corrected upward, and one verification command that returned a directory listing rather than a count now returns the count. The scroll came down from 20,203 pixels to 13,304 without deleting a single screenshot: the flagship and depth sections were 70% of the page purely because every image was stacked full-width, so they are paired into two columns, and LG-Builder's architecture diagram — a screen and a half that every reader scrolled past, including the ones who leave in fifteen seconds — is collapsed behind a summary and one click away. Two defects surfaced while measuring: the hero artefact was rendered a second time inside its own section, which reads as padding, and the two-column grid had a minimum wider than a phone, so the page scrolled sideways on mobile. The five debugging stories are laid out as the post-mortems they already were: the symptom opens across the top, the theory that was wrong sits beside the cause that was real as a matched pair, and the consequence and the receipt close underneath. Five equal columns had given equal weight to five unequal things, and the pair that makes the stories worth reading was just two of the five. Colour now carries the argument — amber for the theory that did not survive, cyan for the one that did — rather than decorating it.
...
### Fixed
- **MCP tools wrote to wherever Forge was launched, not to the project you are working in** — Opened from a Windows shortcut, Forge's working directory is `C:\WINDOWS\system32`, and every tool that writes project state used it: the workflow gate ledger and the change-brief store both tried to write there and were refused by the operating system. The agent saw "Access is denied" and had no way to know the real problem was Forge pointing somewhere nobody works. The project is now resolved when a tool runs rather than when Forge starts, preferring the repository the terminal session is already bound to — the binding exists for the SDD pipeline, so nothing new is tracked. Two hazards surfaced while fixing it, both caught by tests rather than by a user: the search now walks up from a starting directory, because a process rarely sits at the repository root; and the home directory is explicitly refused, because Forge keeps its own state in `~/.forge` and the upward walk would otherwise declare `~` a project and write project state into it. One limitation remains and is deliberate: an MCP request carries no session, so with several repositories open at once the resolver picks one of them. Carrying the session on the request is the narrower fix and is worth doing separately. Proven by driving a real tool with the process standing somewhere unwritable and the project elsewhere — the exact shape of the bug — and asserting that the brief and its gate both reach the project while nothing at all is written where the process happens to be standing.
...
### Added
- **Changes now carry a brief the developer has to see, and a commit without one will be refused** — The working loop had become command, agree, test: explanations were produced but not read, so understanding was never established. A change brief is a small set of short panels — what changed, why, what could break, and the decision that mattered with the alternative it beat — published as a structured document rather than printed as prose. That distinction is the whole design. An agent CLI is a full-screen program, so what reaches the terminal buffer is screen redraws rather than a transcript, and no reliable reading of prose is possible; a published document, by contrast, is either present or absent, and absence is something a commit hook can refuse. Validation is deliberately strict about emptiness, because a gate that an empty document satisfies is a formality — and a formality is exactly what failed here before, when a response-format rule shipped as an advisory reminder and was ignored for months. A change genuinely can be routine, and saying so explicitly is allowed; silence is not, because a developer reads silence as nothing here needed thinking about. The gate itself is one entry added to the list the pre-commit hook already reads, so the mechanism that refuses a commit whose tests were never recorded now also refuses one whose change was never explained. Agents publish through an MCP tool rather than by printing, which is what makes any of this possible and also what makes it provider-agnostic: every CLI under consideration speaks the same protocol, so the guardrail does not care which assistant is running. The gate is scoped to what is actually staged: a documentation-only commit is not asked for a brief, because a gate that fires on everything gets bypassed on everything, and a bypass reached for by reflex is a gate that has quietly stopped working. The brief renders as a billboard rather than a report: three colour-coded panels taken in at a glance, with risk given its own colour because a risk buried in a paragraph is a risk nobody reads, and the decision shown as a fork with both sides visible and an open question at the end. A panel that closes with a statement invites a nod; one that closes with a question invites a challenge, and challenging is how understanding actually forms. A brief published before a page reload is restored, because by then the commit gate is already satisfied and a lost brief would leave the developer believing they had reviewed something they never saw. New Go code is also checked for names that tell a reader nothing: a bare single letter outside a loop iterator or an HTTP handler parameter, and a boolean that does not read as a question. Those refuse the commit. Verb-first function naming is reported but does not block, and the reason is worth recording — recognising a verb needs a word list, and run against this repository the first version rejected Archive, Search, Subscribe, Migrate, Infer and Heal, four hundred findings of which almost all were good names. A checker that rejects good names does not teach better naming; it teaches you to reach for the bypass, after which nothing is enforced at all. A reply that reads as a wall of text now raises a warning when a turn ends, and that check is deliberately the weakest thing here: an agent CLI is a full-screen program, so the terminal buffer holds screen redraws rather than a transcript, and any verdict drawn from it is a guess. A guess must never gate — one wrong block and the bypass becomes reflex — so it warns, says "looks like" rather than asserting, and there is no code path by which it can refuse anything. The brief also scales to the change: a routine edit renders as one panel, and a sprawling one summarises rather than growing without bound, because a brief that expanded with the size of the change would become the thing it replaces. specs/012-compact-visual-style is marked superseded, with the reason it was never built recorded on it.
...
### Added
- **Right-clicking a folder now gives you its path, in both the Projects Browser and the file tree** — Copy Path existed only on *files* in the file tree; a folder's menu never offered it, and project folders in the Projects Browser had no right-click menu at all, so the path a project row is most often wanted for could not be got out of the UI. Folders in both surfaces now offer Copy Path, and the Projects Browser adds Copy Folder Name and Open in Terminal alongside it. Two defects surfaced on the way: the file tree's menu was passed `clientX`/`clientY` while destructuring `x`/`y`, so it opened in the corner rather than under the pointer; and every copy went straight to `navigator.clipboard`, which is undefined outside a secure context — on a plain-HTTP LAN address the copy silently did nothing. Copies now run through a shared helper that falls back to a selection copy and reports whether it worked, and the menu says "Copied path" or that the clipboard was unavailable, so a failure is visible rather than guessed at. The menu itself was extracted from FileExplorer into a shared component so the two surfaces cannot drift apart in positioning, Escape handling, or dismissal.
- **U2 Counter added to the portfolio, leading the depth tier** — An AI answering trade-counter questions against an ERP on a Pick (MultiValue) database, through a hardened fork of the U2 MCP server. It leads the depth tier because its opening line is the strongest on the page — never having heard of Eclipse or a Pick database on Friday afternoon, and having it running by Monday morning — and because that claim is only worth making with the checkable work under it: each fix to the forked server is pinned to a regression test that fails upstream and passes on the fork, and the limits are written down rather than left for a reader to find. Three source-derived replicas: the guided tour spotlighting free-to-sell, the branch table answering a counter question, and the assistant's answer with every MCP call and the raw record beneath it. The product already runs on a synthetic store, and every part, branch, customer and figure was invented again for publication in `u2-counter-demo-data.mjs`; the parsed table is derived from the same record the bytes panel prints, so the two cannot disagree in a published image. The attribute and value marks are the real separator characters, because a replica that drew them as punctuation would be claiming the one thing about this data a relational store could not have produced. No live link is published: the container scales to zero and a twenty-second cold start reads as a broken link. The limits link is labelled simply "What it does not show": the fuller label wrapped to two lines on a phone and stranded its arrow, and what the document contains is already said in the key points above it.
- **Portfolio entries can now carry a headline, key points, and outbound links** — Three optional fields on the app schema, rendered only in the full-width tier. The links are the notable part: an entry carries a repository-relative path rather than a full URL, and the renderer supplies the base, because the generated data file is scanned for the maintainer's handle as a sign that a local path has leaked into published copy — so the link works without weakening the guard that caught a real filename once before.
...
- **Portfolio visuals now use PNG product screens instead of generated SVG storyboard art** — The portfolio asset pipeline now removes the generic SVG fallback, copies real MBL2PC UI captures, renders source-derived PNG replicas for the remaining app states, and validates that every card points to a concrete PNG with visible safe sample data.
- **MBL2PC portfolio story now highlights file handoff and snippets** — Reworked the MBL2PC section so the screenshots and copy show a phone sending a PC-friendly portfolio link, the PC returning a resume PDF, and saved outreach snippets instead of generic text messages.
...
- **LG-Builder section now carries an architecture diagram read off the source** — The five screenshots showed the pipeline behaving; none of them showed it being designed. The section now opens with the compiled graph: every box is a node the orchestrator actually registers, named exactly as it names it, with state travelling between them as `OrchestrationGraphState`. It distinguishes agent nodes from deterministic function nodes and from the human pause, shows the clarification loop returning to validation, and shows the circuit breaker that ends it after three rounds. The SLA escalation and auto-rejection are deliberately drawn *outside* the graph, because they are — the graph has no clock; a separate watcher resolves the suspended run. Observability is a three-row callout rather than a second diagram, since three rows do not need a picture. Built as markup rather than an SVG or Mermaid so it reflows on a phone and keeps the page's type scale; verified at 390px with zero horizontal overflow and no text under 11px. A test checks every drawn node name, the state class, and the round limit against `langgraph_orchestrator.py`, skipping cleanly when that repository is not present.
- **Forge Terminal terminals now fill their pane and read differently at a glance** — The sessions were roughly half the height of the terminal, so each screen showed a band of empty scrollback above the prompt and read as an idle app. Each session is now long enough to fill the pane, with the oldest line clipping at the top the way a terminal scrolled to the bottom actually does, and each has its own colour character: the sandboxed test run is a wall of green passes, the bug recording is amber console warnings, the release job is a progress list, the planning session is a decision table. A test enforces a minimum scrollback length so a screen cannot regress to looking idle.
- **Screenshots now cache-bust, so a redeploy is never invisible** — Asset filenames are stable across rebuilds and GitHub Pages serves them with a ten-minute cache, so a returning visitor kept seeing the previous screenshots after a deploy — the change looked like it had not shipped. Image sources now carry the same kind of version query the JS and data modules already used.
- **Portfolio narrowed to four products, and mbl2pc rebuilt as replicas** — Retired the EZTest and QuiKeys entries. Both sold UX polish while every other entry sells engineering judgement, and a reader's impression is set by the weakest item on the page rather than the average. The remaining four are Forge Terminal, LG-Builder, NodeToolbox, and mbl2pc. mbl2pc was the last section still publishing genuine screenshots — captures of personal devices, one of which showed a file named after a real person — so it is now three source-derived replicas across the app's light, dark, and ocean themes, with copy rewritten to match what the screens actually show rather than the retired résumé narrative. The real-capture copy path and its constants were deleted with the last consumer, and `imageKind` narrowed to replicas only.
- **A private filename was surviving in published data, and now cannot** — The per-app screen suites scan rendered markup, so they could not see a real filename sitting in a field that described how the data had been anonymised. One did, and it reached `web/portfolio/data/apps.mjs`. A new test scans everything the site actually publishes — the generated app data, the narrative data, and the page markup — against the combined list of private strings from every product.
- **LG-Builder added to the portfolio as a second depth entry** — The agentic delivery pipeline built on LangGraph now has its own full-width section, with five surfaces across the two places it actually runs: the chat integration where it stops and asks a human, the SLA escalation and auto-rejection that fire when nobody answers, the review queue that keeps the authoritative record of every pause, the pipeline view showing a production defect fed back into intake, and the automation policy that makes every limit on the machine a setting rather than a code change. The reference material was a real chat workspace and a live admin console, so the server, channels, colleagues, ticket identifiers, issue keys, repository URLs, and escalation webhook are all invented in `lgbuilder-demo-data.mjs`; the webhook is rendered masked so no usable credential is published at all, and a guard test rejects both known private strings and any opaque credential-shaped value while still allowing the product's own snake_case checkpoint identifiers. The depth tier now holds two products, and the registry's hard product-count assertions were replaced with a range so adding or retiring a product no longer breaks the build.
- **NodeToolbox rebuilt as the portfolio's depth tier, with five real surfaces** — Replaced three generic mockups with replicas of the app's most-developed screens: the Roll-Up Board, the daily forecast, the hygiene workspace, Feature composition, and cross-level search. The entry now leads with the product's actual thesis — it reconstructs the truth about delivery and says so when it cannot — rather than the weaker "unifies four systems" breadth claim, and gets its own full-width section between the flagship and the case studies instead of a grid cell. The source captures are live views of a real employer workspace and are never published: every project key, colleague, team, initiative, and host is invented in `nodetoolbox-demo-data.mjs`, and a guard test rejects any that are not, including any issue key not using an invented prefix. A further test pins the three honest-uncertainty surfaces in place so the copy cannot outlive the images that support it.
- **Forge Terminal screens now show one piece of work moving through the workflow** — Each screen carried identical terminal output and an identical phase bar, which read as a static template rather than an application in use. Every screen now has its own session — planning, context selection, a sandboxed test run, a release job, a bug recording — and its own point in the six-stage pipeline, so the set reads as one change advancing from Plan to Implement. The stages are now explained in the copy rather than left as unlabelled jargon. Tests reject duplicate sessions and a phase bar that never advances. Captures are also trimmed to the height their content occupies, so no asset ships with a band of dead space.
