...
- **Mouse clicks and scroll wheel no longer appear as literal escape-sequence text in the terminal**— Root cause: when a session reattaches (`SESSION_REATTACHED`), the server sends the 64 KiB scrollback ring-buffer replay *before* the control message. That replay may contain `\x1b[?1000h` sequences from a TUI that was running; xterm queues and processes them after our mount-time reset, silently re-enabling mouse tracking. A new 500 ms deferred reset fires in the `SESSION_REATTACHED` handler — after all queued replay writes have been processed by xterm — and writes the full mouse-mode disable sequence client-side. An `isMouseEscapeSequence` filter in `onData` provides a belt-and-suspenders guard for the brief race window between queuing the reset and xterm acting on it.
- **Number keys and other printable characters no longer disappear on recovered tabs after an upgrade** — The `isPlainPrintable` redirect in `App.jsx` previously only forwarded characters when the WebSocket was `OPEN`. During the connection window (`CONNECTING`) the key was silently dropped. The redirect now also handles `CONNECTING` via a new `sendOrBuffer` method on the terminal ref, which pushes the character into `pendingInputRef` to be flushed once the socket opens — the same path used by xterm's own `onData` handler.
