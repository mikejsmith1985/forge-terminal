...
- **One-click path fix**: If the PTY spawn test fails because of a non-existent working directory, the wizard fetches the current config, clears `psHomePath`/`cmdHomePath`/`wslHomePath`, saves it back, and re-runs the spawn test to confirm the fix — all in one button press.
- **Manual re-trigger via `triggerWizard('manual')`**: The `useGuidedTour` hook now exposes `triggerWizard` for future Settings integration.
