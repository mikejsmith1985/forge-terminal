...
### Fixed
- **Four more sources of the terminal CMD-window flash.** `detect.go` tailscale status poll (fires every 10s while the ConnectionSetupCard is visible), `service.go` `sc query cloudflared` (fires on every setup-status poll), `service.go` service install/uninstall, and `tunnel.go` tailscale funnel reset were still using the inline `exec.CommandContext(...).Run/Output()` pattern that skips `hideWindow()`.  All rewritten to named-variable form with `hideWindow(cmd)` applied before execution.
